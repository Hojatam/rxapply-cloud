...
// =====================================================================
// F9 · Cost telemetry + monthly cap enforcement.
//
// Two cost sources:
...
function getCap() {
  try {
    const out = _psql(`SELECT monthly_cap_usd FROM dashboard_settings WHERE id = 1;`);
...
// caching that's 2 docker subprocess calls × ~3 callers per minute. The
// cap-enforcement path (canSpend) can also be served from cache; a few
// seconds of staleness is harmless for a $25/month budget guard.
...
function byAgent(days = 30, limit = 8) {
  const sql = `
    WITH all_costs AS (
    ...
    SELECT json_agg(row_to_json(t) ORDER BY t.cost DESC) FROM (
...
function snapshot() {
  ...
  const v = _freshSnapshot();
  _snapCache = { value: v, until: now + SNAPSHOT_TTL_MS };
  return v;
}

function canSpend(projectedAddUsd = 0) {
  const snap = snapshot();
